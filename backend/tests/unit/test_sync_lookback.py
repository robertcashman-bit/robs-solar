...
def test_quickfile_force_full_lookback_is_about_two_years() -> None:
    now = datetime(2026, 8, 18, 12, 0, tzinfo=timezone.utc)
    assert quickfile_lookback_days(force_full=True) == QUICKFILE_FIRST_SYNC_LOOKBACK_DAYS
    assert QUICKFILE_FIRST_SYNC_LOOKBACK_DAYS == 730
    assert quickfile_lookback_since(force_full=True, now=now) == "2024-08-18"
